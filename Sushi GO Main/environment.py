...
class Player(object):
    ...
    def choose1Wisely(self):
        #if 1 tempura chosen, pick another one to pair up
        if self.cards['Tempura'] % 2 == 1 and 'Tempura' in self.hand:
            return 'Tempura'
        #if 1 tempura chosen, pick another one to pair up
        elif self.cards['Sashimi'] % 3 == 2 and 'Sashimi' in self.hand:
            return 'Sashimi'
        #if wasabi chosen, pick nigiri first (squid > salmon; don't choose egg)
        elif len(self.cardSeq) == 0 or self.cardSeq[-1] == 'Wasabi':
            if 'Squid Nigiri' in self.hand:
                return 'Squid Nigiri'
            elif 'Salmon Nigiri' in self.hand:
                return 'Salmon Nigiri'
            else:
                return self.choose1Randomly()
        else:
            return self.choose1Randomly()

    def choose2Wisely(self):
        ...
        elif 'Wasabi' in self.hand and 'Squid Nigiri' in self.hand:
            return 'Wasabi','Squid Nigiri'
        elif 'Wasabi' in self.hand and 'Salmon Nigiri' in self.hand:
            return 'Wasabi','Salmon Nigiri'
        else:
            return self.choose2Randomly()
...
class Game(object):
    ...
    def chooseElseWisely(self,app):
        for player in app.players[1:]:
            if player.chopsticksNotUsed:
                card1,card2 = player.choose2Wisely()
                player.useChopsticks(card1,card2)
            else:
                card = player.choose1Wisely()
                player.chooseCard(card)
